/**
 * Component: ExportBlotterModal
 * Modal configuration UI for exporting Blotter Case records to Excel (.xlsx).
 */
...
import {
  ...
  BlotterExportFilterParams,
  BlotterExportDateParams,
  validateDateRange,
  filterBlottersForExport,
  exportBlottersToExcel,
} from '../../utils/excelBlotterExport';
import {
  FileSpreadsheet,
  Calendar,
  Filter,
...
export interface ExportBlotterModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const ExportBlotterModal: React.FC<ExportBlotterModalProps> = ({
  isOpen,
  onClose,
  rawCases,
  pageFilters,
  ...
  const [overrideStatusFilter, setOverrideStatusFilter] = useState<boolean>(false);
  ...
  const dateParams: BlotterExportDateParams = useMemo(
    () => ({
      dateScope,
      selectedMonth,
      selectedYear,
  ...
  // Consolidated page filter bundle
  const effectivePageFilters: BlotterExportFilterParams = useMemo(
    () => ({
      searchQuery: pageFilters.searchQuery,
      statusFilter: pageFilters.statusFilter,
      overrideStatusFilter,
    }),
    [pageFilters.searchQuery, pageFilters.statusFilter, overrideStatusFilter]
  );
  ...
  // Computed blotter cases matching active page filters + export date scope
  const matchingCases = useMemo(() => {
    return filterBlottersForExport(rawCases, effectivePageFilters, dateParams);
  }, [rawCases, effectivePageFilters, dateParams]);

  // Readable label for status filter preview
  const getStatusLabel = (filterVal: string) => {
    switch (filterVal) {
      case 'all':
        return 'All Cases';
      case 'open':
        return 'Open / Under Investigation';
      case 'scheduled':
      ...
      case 'cfa':
        return 'CFA Issued';
      default:
        return filterVal.charAt(0).toUpperCase() + filterVal.slice(1);
    }
  };
  ...
  const handleExecuteExport = () => {
    ...
    if (matchingCases.length === 0) {
      setExportMessage({
        type: 'error',
        text: 'No blotter cases match the selected filters.',
      });
      return;
    }

    const result = exportBlottersToExcel(matchingCases, dateParams);

    if (result.success) {
      setExportMessage({
        type: 'success',
        text: `Successfully exported ${result.exportedCount} blotter records to ${result.filename}.`,
      });
      setTimeout(() => {
        onClose();
      }, 1200);
      ...
      setExportMessage({
        type: 'error',
        text: result.error || 'Failed to export blotter records.',
      });
    }
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Export Blotter Records to Excel"
      description="Generate a detailed spreadsheet (.xlsx) of authorized blotter case records matching active filters."
      size="lg"
      footer={
        <div className="flex items-center justify-between w-full">
          <div className="text-xs text-slate-500 font-medium">
            {matchingCases.length > 0 ? (
              <span className="text-emerald-700 font-bold flex items-center gap-1">
                ...
                {matchingCases.length} {matchingCases.length === 1 ? 'record' : 'records'} ready to export
              </span>
            ) : (
              <span className="text-amber-700 font-semibold flex items-center gap-1">
                <AlertTriangle className="w-3.5 h-3.5 text-amber-600" />
                0 records match selection
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              disabled={
                matchingCases.length === 0 || (dateScope === 'custom' && !rangeValidation.isValid)
              }
      ...
      <div className="space-y-5">
        ...
        <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3.5 space-y-2.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
            <div className="bg-white p-2.5 rounded-lg border border-slate-200/60">
              <span className="text-[11px] text-slate-400 block font-medium">Status Filter</span>
              <span className="font-semibold text-slate-800 truncate block">
                {overrideStatusFilter
                  ? 'All Status Types (Override Active)'
                  : getStatusLabel(pageFilters.statusFilter)}
              </span>
            </div>

            <div className="bg-white p-2.5 rounded-lg border border-slate-200/60">
              <span className="text-[11px] text-slate-400 block font-medium">Search Query</span>
              <span className="font-semibold text-slate-800 truncate block">
                {pageFilters.searchQuery.trim() ? `"${pageFilters.searchQuery}"` : 'None (All Records)'}
              </span>
            </div>
          </div>

          {/* Status Override Checkbox */}
          <div className="pt-1 flex items-center justify-between bg-white/80 p-2 rounded-lg border border-slate-200/50">
            <label className="text-xs text-slate-700 font-medium flex items-center gap-2 cursor-pointer select-none">
              <input
                type="checkbox"
                checked={overrideStatusFilter}
                onChange={(e) => {
                  setOverrideStatusFilter(e.target.checked);
                  setExportMessage(null);
                }}
                className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
              />
              <span>Include all blotter status types (ignore current status tab filter)</span>
            </label>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold">All Blotters</span>
                <div
              ...
              <p className="text-[11px] text-slate-500 mt-1 leading-tight">
                Export all authorized blotter cases without date restrictions.
              </p>
            </button>
            ...
            >
              ...
              <p className="text-[11px] text-slate-500 mt-1 leading-tight">
                Filter cases by a specific calendar month and year.
              </p>
            </button>
          ...
          {/* Conditional Controls based on Date Scope */}
          {dateScope === 'month' && (
          ...
          {dateScope === 'custom' && (
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3 animate-in fade-in duration-200">
              ...
              {/* Range Validation Live Feedback */}
        ...
        {matchingCases.length === 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-900 flex items-start gap-2.5">
            ...
            <div>
              <p className="font-bold">No blotter cases match the selected filters.</p>
              <p className="mt-0.5 text-amber-700">
                Try enabling "Include all blotter status types" or select a different date scope.
              </p>
            </div>
          </div>
        )}

        {/* Export Message Feedback */}
        {exportMessage && (
          <div
