/**
 * Component: ExportReportsModal
 * Configuration modal for exporting Report History records to Excel (.xlsx).
 */
...
import {
  ...
  ReportExportFilterParams,
  ReportExportDateParams,
  validateDateRange,
...
export const ExportReportsModal: React.FC<ExportReportsModalProps> = ({
  ...
  // Construct current date parameters
  const dateParams: ReportExportDateParams = useMemo(
    () => ({
      dateScope,
      selectedMonth,
      selectedYear,
  ...
  // Computed reports matching active page filters + export date scope
  const matchingReports = useMemo(() => {
    return filterReportsForExport(rawReports, pageFilters, dateParams);
  }, [rawReports, pageFilters, dateParams]);

  // Readable labels for current page filters summary
  const purokLabel = pageFilters.purokFilter === 'all' ? 'All Sitio/Purok' : pageFilters.purokFilter;
  const categoryLabel =
    pageFilters.categoryFilter === 'all'
      ? 'All Incident Categories'
      : INCIDENT_CATEGORIES.find((c) => c.id === pageFilters.categoryFilter)?.label || pageFilters.categoryFilter;
  const historyTabLabel =
    pageFilters.historyTabFilter === 'all_history'
      ? 'All History Statuses'
      : pageFilters.historyTabFilter.charAt(0).toUpperCase() + pageFilters.historyTabFilter.slice(1);
  ...
  const handleExecuteExport = () => {
    ...
    const result = exportReportsToExcel(matchingReports, dateParams);

    if (result.success) {
      setExportMessage({
        type: 'success',
        text: `Successfully exported ${result.exportedCount} records to ${result.filename}.`,
      });
      setTimeout(() => {
        onClose();
      }, 1200);
      ...
      setExportMessage({
        type: 'error',
        text: result.error || 'Failed to export reports.',
      });
    }
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Export Report History to Excel"
      description="Generate a spreadsheet (.xlsx) of archived report logs matching active filters."
      size="lg"
      footer={
        <div className="flex items-center justify-between w-full">
          <div className="text-xs text-slate-500 font-medium">
            {matchingReports.length > 0 ? (
              <span className="text-emerald-700 font-bold flex items-center gap-1">
                ...
                {matchingReports.length} {matchingReports.length === 1 ? 'record' : 'records'} ready to export
              </span>
            ) : (
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              disabled={
                matchingReports.length === 0 || (dateScope === 'custom' && !rangeValidation.isValid)
              }
      ...
      <div className="space-y-5">
        ...
        <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3.5 space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
            <div className="bg-white p-2 rounded-lg border border-slate-200/60">
              <span className="text-[11px] text-slate-400 block font-medium">Sitio / Purok</span>
              <span className="font-semibold text-slate-800 truncate block">{purokLabel}</span>
            </div>
            <div className="bg-white p-2 rounded-lg border border-slate-200/60">
              <span className="text-[11px] text-slate-400 block font-medium">Category</span>
              <span className="font-semibold text-slate-800 truncate block">{categoryLabel}</span>
            </div>
            <div className="bg-white p-2 rounded-lg border border-slate-200/60 col-span-2 sm:col-span-1">
              <span className="text-[11px] text-slate-400 block font-medium">History Status</span>
              <span className="font-semibold text-slate-800 truncate block">{historyTabLabel}</span>
            </div>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold">All Reports</span>
                <div
              ...
              <p className="text-[11px] text-slate-500 mt-1 leading-tight">
                All records matching current filters without date restrictions.
              </p>
            </button>
        ...
        {/* Export Outcome Feedback / Empty State Alert */}
        {matchingReports.length === 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-900 flex items-start gap-2.5">
            ...
            <div>
              <p className="font-bold">No reports match the selected filters.</p>
              <p className="mt-0.5 text-amber-700">
                Please adjust the page filters or select a different date scope to find records for export.
              </p>
            </div>
          </div>
        )}
