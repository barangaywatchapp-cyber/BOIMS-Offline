/**
 * Phase 6 — Dead Letter Queue, Retry/Backoff & Permanent Failure Management Test Runner
 * Executes all Phase 6 validation tests and verifies Phase 5, Phase 4, and Phase 3 non-regression.
 */
...
async function main() {
  console.log('===========================================================');
  console.log('  BOIMS Phase 6 — Dead Letter Queue & Failure Management');
  console.log('===========================================================\n');

  // 1. Run Phase 6 Test Suite
  console.log('--- Executing Phase 6 DLQ & Failure Management Suite ---');
  const phase6Summary = await runPhase6TestSuite();

  for (const res of phase6Summary.results) {
    if (res.passed) {
  ...
  console.log('\n===========================================================');
  console.log(`Phase 6 Results: ${phase6Summary.passed}/${phase6Summary.total} Passed, ${phase6Summary.failed} Failed`);
  console.log(`Phase 5 Regression: ${phase5Summary.passed}/${phase5Summary.total} Passed`);
  console.log(`Phase 4 Regression: ${phase4Summary.passed}/${phase4Summary.total} Passed`);
  console.log(`Phase 3 Regression: ${phase3Summary.passed}/${phase3Summary.total} Passed`);
  const grandTotal = phase6Summary.total + phase5Summary.total + phase4Summary.total + phase3Summary.total;
  const grandPassed = phase6Summary.passed + phase5Summary.passed + phase4Summary.passed + phase3Summary.passed;
  console.log(`Total Validation: ${grandPassed}/${grandTotal} Passed`);
  console.log('===========================================================');

  if (phase6Summary.failed > 0 || phase5Summary.failed > 0 || phase4Summary.failed > 0 || phase3Summary.failed > 0) {
    process.exit(1);
  }
}
